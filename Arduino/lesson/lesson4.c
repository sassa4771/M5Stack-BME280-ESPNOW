/*
 * lesson4.c
 * ESP-NOW受信 → グラフ表示（M5Stack側）
 * gateway_espnow_serial.inoをベースにグラフ表示機能を追加
 */
...
// ==== lesson4 で追加 ====
// グラフを表示するための設定
const int GRAPH_HISTORY_SIZE = 30;  // グラフに表示するデータの数
const int GRAPH_X_START = 0;        // グラフの左端の位置
const int GRAPH_Y_START = 65;       // グラフの上端の位置
const int GRAPH_WIDTH = 320;        // グラフの幅
const int GRAPH_HEIGHT = 40;        // グラフの高さ

// 過去のデータを保存する配列（温度、湿度、気圧それぞれ）
...
void onRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    ...
    // ==== lesson4 で追加 ====
    // 受信したデータを履歴配列に保存
    tempHistory[historyIndex] = pl.t; // 温度
    humHistory[historyIndex] = pl.h;  // 湿度
    presHistory[historyIndex] = pl.p;  // 気圧
    historyIndex++;
    // 配列の最後まで来たら最初に戻る
