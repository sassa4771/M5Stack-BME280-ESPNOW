...
2. [2. Arduino IDEの設定](#2-arduino-ideの設定)
3. [3. Python環境の準備 (Visual Studio Code)](#3-python環境の準備-visual-studio-code)
4. [4. センサーと通信方式について](#4-センサーと通信方式について)
5. [5. lesson1: センサーを使う](#5-lesson1-センサーを使う)
6. [6. lesson2: マイコンのUI作成](#6-lesson2-マイコンのui作成)
7. [7. lesson3: 無線通信をする（ESP-NOW）](#7-lesson3-無線通信をするesp-now)
8. [8. lesson4: マイコンのUI作成（M5Stack側）](#8-lesson4-マイコンのui作成m5stack側)
9. [9. lesson5: PCとの接続（Python）](#9-lesson5-pcとの接続python)
10. [10. lesson6: PythonでUIを作成](#10-lesson6-pythonでuiを作成)
11. [11. lesson7: 参加者で共同作品](#11-lesson7-参加者で共同作品)
12. [12. 発展編](#12-発展編)
13. [よくあるエラーと解決方法](#よくあるエラーと解決方法)
...
#### 7.3.1. パート1: ゲートウェイ（受信側：M5Stack）の設定
...
> 
> [lesson5のVSCodeでのPythonデータ取得](#94-vscodeでのpythonデータ取得)のセクションを参照してください。
...
AIを使って、以下のように3つのグラフを一つのグラフに3つのデータが表示されるコードを作成してください。今回はマイクロソフトのEdgeを使ってください。AIを使ってみてください。
...
### 12.2. オンライン化（Google Spread Sheet）

lesson5やlesson6で取得したセンサーデータを、Google Spread Sheetに自動で記録して、オンラインで共有・可視化できるようにしてみましょう。

**このセクションで学ぶこと：**
- Google Apps Script (GAS) を使ったデータの自動記録
- HTTP POSTリクエストによるデータ送信
...
**実装の流れ：**
1. Google Spread Sheetを作成し、データを記録するシートを準備
2. Google Apps ScriptでWebアプリを作成し、POSTリクエストを受け取ってスプレッドシートに書き込む処理を実装
3. Webアプリとして公開し、URLを取得
4. Pythonスクリプトを修正して、シリアル通信で取得したデータをHTTP POSTでGASに送信
5. スプレッドシート上でグラフを作成して、リアルタイムでデータを可視化

> [!TIP]
> **参考手順：**
> 1. Google Spread Sheetを作成し、1行目にヘッダー（日時、温度、湿度、気圧など）を設定
> 2. スプレッドシートの「拡張機能」→「Apps Script」を開く
> 3. GASでPOSTリクエストを受け取り、スプレッドシートに追記する関数を作成
> 4. 「デプロイ」→「新しいデプロイ」でWebアプリとして公開（「全員」にアクセス権限を付与）
> 5. Pythonで`requests`ライブラリを使用して、取得したデータをPOSTリクエストで送信
> 6. スプレッドシート上でグラフを作成して、リアルタイムでデータの変化を確認

> [!NOTE]
> **API方式との比較：**
> 
> Google Spread Sheetにデータを書き込む方法には、**Google Sheets API**を使う方法もあります。API方式では、Google Cloud Platformでプロジェクトを作成し、サービスアカウントを作成してJSONキーを取得する必要があります。また、`gspread`などのライブラリを使用して認証を行う必要があります。
> 
> 一方、**GASを使ったPOST方式**のメリットは以下の通りです：
> - **認証が不要**：APIキーやJSONキーの管理が不要で、URLにPOSTリクエストを送るだけでデータを書き込めます
> - **設定が簡単**：Google Cloud Platformでの設定が不要で、スプレッドシート内で完結します
> - **セキュリティ設定が柔軟**：GAS側で簡単な認証トークンやパスワードを実装することも可能です
> - **初心者にも優しい**：複雑な認証フローを理解する必要がなく、すぐに実装できます
