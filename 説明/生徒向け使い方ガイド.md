...
6. 検索欄に「**ESP32**」と入力
7. 「**esp32 by Espressif Systems**」を探して、**バージョン2.0.1**を選択して**インストール**をクリック
...
2. 検索欄にライブラリ名を入力して、それぞれインストール：
   ...
   **M5StickCPlus**（M5StickC Plus用）
   - 検索: 「M5StickCPlus」
   - 「M5StickCPlus by M5Stack」をインストール

   **M5Stack**（M5Stack用）
   - 検索: 「M5Stack」
   - 「M5Stack by M5Stack」をインストール

   **Adafruit BME280 Library**
   - 検索: 「BME280」
   - 「Adafruit BME280 Library by Adafruit」をインストール

#### 4. Python環境の準備（lesson5-7用）
...
**📷 画像: シリアルモニターの画面**
- ファイル名: `serial_monitor_example.png`
- 説明: Arduino IDEのシリアルモニター画面
- JSON形式のデータが表示されている
...
1. Arduino IDEを開く
2. **ファイル > 開く**をクリック
3. プロジェクトフォルダの`Arduino/lesson/lesson1.c`を選択
4. ファイルが開きます
...
- この下に自分の番号を表示する行を追加してみましょう
- 例: `M5.Lcd.printf("No: %d\n", 5);`（5を自分の番号に変更）
...
1. **ファイル > 開く**で`Arduino/lesson/lesson2.c`を開く
...
**注意**: 変更するときは、`drawGraph(temp);`の`temp`の部分だけを変更してください。`drawGraph(`と`);`はそのままにしてください。
...
1. M5StickCの画面に「ESP-NOW Sender」と表示され、送信状態が表示されます
2. M5StackのシリアルモニターにJSON形式のデータが表示されます
   - 例: `{"type":"sample","id":"B1","t":25.30,"h":55.20,"p":1013.25,"seq":123,"from":"98:f4:ab:6c:e7:88"}`

![[84723.jpg]]

**📷 画像26: lesson3の動作確認（受信側）**
- ファイル名: `lesson3_receiver.png`
- 説明: M5Stackのシリアルモニター画面
- JSON形式のデータが1秒ごとに表示されている
...
**📷 画像27: ESP-NOWのコード箇所**
- ファイル名: `espnow_code_locations.png`
- 説明: lesson3.cのコードエディタ画面
- `esp_now_init()`, `esp_now_send()`, `OnDataSent()`の3箇所に番号と説明が付いている
...
1. lesson3のパート1と同じように、M5Stackに`Arduino/lesson/lesson4.c`をアップロード
2. lesson3のパート2と同じように、M5StickCに`Arduino/lesson/lesson3.c`をアップロード
...
**📷 画像29: コードの変更箇所（lesson4）**
- ファイル名: `lesson4_code_edit.png`
- 説明: lesson4.cのコードエディタ画面
- 温度のグラフを描画している部分と、その下に追加する湿度・気圧のグラフのコードが表示されている
...
**📷 画像30: デバイスマネージャーでポート確認（Windows）**
- ファイル名: `device_manager_port.png`
- 説明: Windowsのデバイスマネージャー画面
- 「ポート（COMとLPT）」を展開し、COM5などが表示されている
...
**📷 画像31: lesson5のシリアルモニター**
- ファイル名: `lesson5_serial_monitor.png`
- 説明: Arduino IDEのシリアルモニター画面
- JSON形式のデータが1秒ごとに表示されている
...
**📷 画像32: コマンドプロンプトでPython実行**
- ファイル名: `python_command.png`
- 説明: コマンドプロンプト/ターミナルの画面
- `python lesson6.py --port COM5`を実行している状態
...
**📷 画像34: lesson7の準備物（複数人）**
- ファイル名: `lesson7_materials.png`
- 説明: 複数のM5StickC Plus、1台のM5Stack、BME280センサー、USBケーブルを並べた写真
...
1. **ファイル > 開く**で`Arduino/lesson/lesson7.c`を開く
2. **ツール > ボード**で「**M5StickC Plus**」を選択
...
3. **全員が同じ値**に設定する部分：
   ...
   - `GATEWAY_MAC`: パート1で確認したMACアドレスに変更（lesson3と同じ方法）
...
**📷 画像35: lesson7のコード変更箇所**
- ファイル名: `lesson7_code_edit.png`
- 説明: lesson7.cのコードエディタ画面
- `CHANNEL`、`GATEWAY_MAC`、`DEVICE_ID`の3箇所に番号と説明が付いている
...
2. プロジェクトフォルダの`python`フォルダに移動：
   ```bash
   cd python
   ```
3. 可視化ツールを起動：
   ```bash
   python pc_viewer_2dmap_interpolated.py --port COM5 --metric t  # 温度マップ（COM5を自分のポートに変更）
   # または
...
**📷 画像36: lesson7の2Dマップ表示**
- ファイル名: `lesson7_2dmap.png`
- 説明: PC画面に表示された2Dマップ
- 4x4グリッドで各座席の温度が色で表現されている
- 座席ID（A1, A2, B1など）が表示されている
...
- `displayMetric`変数を変更して、気圧や湿度のグラフを表示
  - コードの上の方に`char displayMetric = 't';`という行があります
  - `'t'`を`'h'`（湿度）や`'p'`（気圧）に変更できます
- 送信間隔を変更（`SEND_INTERVAL_MS`）
  - `const int SEND_INTERVAL_MS = 1000;`の`1000`を変更すると、送信間隔が変わります（単位: ミリ秒）
...
**📷 画像37: ボードが見つからないエラー**
- ファイル名: `error_board_not_found.png`
- 説明: Arduino IDEのボード選択画面
- 「M5StickC Plus」が表示されていない状態
...
**📷 画像38: シリアルポートが見つからないエラー**
- ファイル名: `error_port_not_found.png`
- 説明: Arduino IDEのシリアルポート選択画面
- ポートが表示されていない状態
...
**📷 画像39: ライブラリエラー**
- ファイル名: `error_library.png`
- 説明: Arduino IDEのコンパイルエラー画面
- 「'M5StickCPlus' was not declared」というエラーメッセージが表示されている
...
**📷 画像40: アップロードエラー**
- ファイル名: `error_upload.png`
- 説明: Arduino IDEのアップロードエラー画面
- 「Failed to connect to ESP32」というエラーメッセージが表示されている
...
**📷 画像41: BME280の配線図**
- ファイル名: `bme280_wiring.png`
- 説明: M5StickC PlusとBME280の接続図
- SDA、SCL、VCC、GNDの接続が図示されている
...
**📷 画像42: ESP-NOW通信の確認方法**
- ファイル名: `espnow_troubleshooting.png`
- 説明: コードエディタとシリアルモニターを並べた画面
- `CHANNEL`と`GATEWAY_MAC`の設定箇所に矢印が付いている
...
**📷 画像43: Pythonエラー**
- ファイル名: `error_python.png`
- 説明: コマンドプロンプト/ターミナルの画面
- Pythonのエラーメッセージが表示されている
...
以下の画像を用意してください。各画像には説明とファイル名を記載しています。

1. **arduino_ide_screen.png** - Arduino IDEの全体画面
2. **hardware_setup.png** - 必要なハードウェア（M5StickC Plus、M5Stack、BME280、USBケーブル）
3. **arduino_download.png** - Arduino IDE公式サイトのダウンロードページ
4. **arduino_preferences.png** - Arduino IDEの環境設定画面
5. **board_manager.png** - ボードマネージャーの画面
6. **esp32_install.png** - ESP32のインストール画面
7. **library_manager.png** - ライブラリマネージャーの画面
8. **m5stickcplus_library.png** - M5StickCPlusライブラリの検索結果
9. **python_install.png** - Pythonライブラリのインストール画面
10. **bme280_sensor.png** - BME280センサーの外観（ピン配置がわかる写真）
11. **i2c_connection.png** - BME280とM5StickC PlusのI2C接続図
12. **espnow_concept.png** - ESP-NOW通信の概念図（送信側と受信側の関係）
13. **serial_monitor_example.png** - シリアルモニターの画面例
10. **lesson1_materials.png** - lesson1の準備物
11. **open_file.png** - ファイルを開く画面
12. **select_board.png** - ボードの選択画面
13. **select_port.png** - シリアルポートの選択画面
14. **m5stickc_connected.png** - M5StickC PlusをUSBケーブルで接続している写真
15. **compile_success.png** - コンパイル成功画面
16. **upload_success.png** - アップロード成功画面
17. **lesson1_result.png** - lesson1の動作画面（M5StickCの画面）
18. **lesson1_code_edit.png** - lesson1のコード変更箇所
19. **lesson2_result.png** - lesson2の動作画面（グラフが表示されている）
20. **lesson2_code_edit.png** - lesson2のコード変更箇所
21. **lesson3_materials.png** - lesson3の準備物
22. **serial_monitor_mac.png** - シリアルモニターでMACアドレスを確認している画面
23. **mac_address_change.png** - MACアドレスの変更方法
24. **device_id_change.png** - デバイスIDの変更方法
25. **lesson3_sender.png** - lesson3の送信側（M5StickC）の画面
26. **lesson3_receiver.png** - lesson3の受信側（M5Stack）のシリアルモニター
27. **espnow_code_locations.png** - ESP-NOWのコード箇所
28. **lesson4_result.png** - lesson4の動作画面（M5Stackの画面）
29. **lesson4_code_edit.png** - lesson4のコード変更箇所
30. **device_manager_port.png** - Windowsのデバイスマネージャーでポートを確認している画面
31. **lesson5_serial_monitor.png** - lesson5のシリアルモニター画面
32. **python_command.png** - コマンドプロンプトでPythonを実行している画面
33. **lesson6_graph.png** - lesson6のグラフ表示画面
34. **lesson7_materials.png** - lesson7の準備物（複数人分）
35. **lesson7_code_edit.png** - lesson7のコード変更箇所
36. **lesson7_2dmap.png** - lesson7の2Dマップ表示画面
37. **error_board_not_found.png** - ボードが見つからないエラー画面
38. **error_port_not_found.png** - シリアルポートが見つからないエラー画面
39. **error_library.png** - ライブラリエラー画面
40. **error_upload.png** - アップロードエラー画面
41. **bme280_wiring.png** - BME280の配線図
42. **espnow_troubleshooting.png** - ESP-NOW通信の確認方法
43. **error_python.png** - Pythonエラー画面
