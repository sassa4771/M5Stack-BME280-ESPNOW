...
![Arduino IDE画面](attachments/arduino_ide_screen.png)
...
- **BME280センサー**（I2C接続）
- **USBケーブル**（データ転送対応のもの）
![M5StickC PlusとM5Stack](attachments/hardware_m5stick_m5stack.png)
...
![Arduino IDEダウンロード](attachments/arduino_ide_download.png)
...
![Arduino環境設定](attachments/arduino_preferences.png)
- 説明: Arduino IDEの環境設定画面
- 「追加のボードマネージャーのURL」の欄が表示されている

**ステップ2: ボードマネージャーのURLを追加**

![ボードマネージャーURL追加](attachments/board_manager_url.png)
...
![ボードマネージャーを開く](attachments/board_manager_open.png)
...
![ESP32とM5Stackインストール確認](attachments/esp32_m5stack_installed.png)
...
![ライブラリマネージャー](attachments/library_manager.png)
...
![I2C配線図](attachments/i2c_wiring.png)
...
![シリアルモニター画面](attachments/serial_monitor.png)
...
![M5StickC PlusとBME280](attachments/m5stick_bme280.jpg)
...
![Lesson1 ファイルを開く](attachments/lesson1_open_file.png)

#### ステップ2: ボードを選択
1. **ツール > ボード**をクリック
2. **M5Stack>M5StickCPlus**を選択

![ボード選択](attachments/board_selection.png)
...
![シリアルポート選択](attachments/serial_port_selection.png)

#### ステップ4: コンパイルとアップロード

1. **スケッチ > 書き込み**をクリック（または`Ctrl+U`/`Cmd+U`）
...
**作成結果**
![Lesson1 実行結果](attachments/lesson1_result.png)
...
![Lesson2 グラフ表示](attachments/lesson3_running.jpg)
...
![ESP-NOW通信の構成](attachments/espnow_setup.png)

### セットアップ手順

#### パート1: ゲートウェイ（受信側：M5Stack）の設定

1. **ファイル > 開く**で`Arduino/lesson/lesson3.1.ino`を開く
2. **ツール > ボード**で「**M5Stack-Core-ESP32**」を選択
   ![M5Stack-Core-ESP32選択](attachments/m5stack_core_selection.png)
...
1. M5StickCの画面にデバイスIDやセンサーの値、送信状態が表示されます
2. M5StackのシリアルモニターにJSON形式のデータが表示されます
   - 例: `{"type":"sample","id":"B1","t":25.30,"h":55.20,"p":1013.25,"seq":123,"from":"98:f4:ab:6c:e7:88"}`

![Lesson3 動作確認](attachments/lesson3_running.jpg)

![Lesson3 シリアルモニター](attachments/lesson3_serial_monitor.png)
### ★参加者ミッション
...
1. lesson3のパート1と同じように、M5Stackに`Arduino/lesson/lesson4.ino`をアップロードしてください。
...
![Lesson4 グラフ表示](attachments/lesson5.png)
### ★参加者ミッション
...
1. lesson3のパート1と同じように、M5Stackに`Arduino/lesson/lesson5.ino`をアップロード
2. lesson3のパート2と同じように、M5StickCに`Arduino/lesson/lesson3.ino`をアップロード(レッスン3から変更しなくてよいです)
3. M5StackをUSBケーブルでPCに接続
4. シリアルポート番号を確認: COMポートはM5Stack（灰色の大きいマイコン）のポートを指定してください。
   - Windows: デバイスマネージャーで「COM5」など
   - Mac: ターミナルで`ls /dev/tty.*`を実行
![シリアルモニタの開き方](attachments/serial_monitor_menu.png)
...
1. VSCodeを起動
2. **ファイル > フォルダーを開く** で、今回の講義のフォルダ（`M5Stack-BME280-ESPNOW`）を開く
...
![VSCodeターミナルを開く](attachments/vscode_terminal_open.png)

ターミナルが開くと、VSCodeの下部にコマンド入力エリアが表示されます。

#### ステップ3: pythonフォルダに移動

ターミナルで以下のコマンドを入力して、Enterキーを押します：
...
- `python`は移動先のフォルダ名
- このコマンドで、プロジェクトフォルダ内の`python`フォルダに移動します

![VSCodeターミナルでコマンド実行](attachments/vscode_terminal_command.png)
...
COMポートはM5Stack（灰色の大きいマイコン）のポートを指定してください。

**Windowsの場合:**
```bash
python lesson5.py --port COM5
```
**COM5**の部分を自分のポート番号に変更してください（例: COM3, COM4, COM7など）
- ポート番号がわからない場合は、Arduino IDEの**ツール > シリアルポート**で確認できます

**macOS/Linuxの場合:**
```bash
python lesson5.py --port /dev/tty.usbserial-xxxx
```
**xxxx**の部分を自分のポート番号に変更してください
- ポート番号がわからない場合は、ターミナルで`ls /dev/tty.*`を実行して確認できます
...
- エラーが出た場合は、ポート番号が正しいか確認
- 他のアプリケーション（Arduino IDEのシリアルモニターなど）がポートを使用していないか確認
- Pythonがインストールされていない場合は、エラーメッセージが表示されます。その場合は[必要な環境の準備](#必要な環境の準備)を確認してください
...
> 
> [lesson5のVSCodeでのPythonデータ取得](#vscodeでのpythonデータ取得)のセクションを参照してください。
> ターミナルの開き方、`cd python`コマンドの実行方法、ポート番号の確認方法など、詳しく説明しています。
### 動作確認
...
![Lesson6 グラフ表示](attachments/pythonG.png)

### ★参加者ミッション

AIを使って、3つのグラフを一つのグラフに3つのデータが表示されるコードを作成してください。今回はマイクロソフトのEdgeを使ってください。ChatGPTを使ってください。
...
1. lesson3のパート1と同じように、M5Stackに`Arduino/lesson/lesson5.c`をアップロードしてください
...
![デバイスマネージャー](attachments/device_manager.png)
