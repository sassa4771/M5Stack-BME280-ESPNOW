...
### Pythonでのデータ取得
VSCodeのターミナルで以下のコードを実行してください。COMポートはM5Stack(灰色の大きいマイコン)のポートを指定してください。
   ```bash
   python lesson5.py　--port COM4
   ```
...
3. プロジェクトフォルダの`python`フォルダに移動：
   ...
   cd python
   ```
4. Pythonスクリプトを実行：COMポートはM5Stack(灰色の大きいマイコン)のポートを指定してください。
   ```bash
